...
namespace FermentaLabOnion.API.Controllers
{
    ...
    [ApiController]
    public class ShareSpecialsController : ControllerBase
    {
        private readonly IShareSpecialService _service;

        public ShareSpecialsController(IShareSpecialService service)
        {
            _service = service;
        }
        [HttpGet]
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromForm] ShareSpecialCreateDto productDto)
        {
            await _service.CreateAsync(productDto);
            return StatusCode(StatusCodes.Status200OK);
        }
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync([FromForm] ShareSpecialUpdateDto updateDto, int id)
        {
            if (id <= 0) return StatusCode(StatusCodes.Status400BadRequest);
