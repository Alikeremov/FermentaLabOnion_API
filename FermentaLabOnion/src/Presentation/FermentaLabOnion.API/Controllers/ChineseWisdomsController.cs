...
namespace FermentaLabOnion.API.Controllers
{
    ...
    [ApiController]
    public class ChineseWisdomsController : ControllerBase
    {
        private readonly IChineseWisdomService _service;

        public ChineseWisdomsController(IChineseWisdomService service)
        {
            _service = service;
        }
        [HttpGet]
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromForm] ChineseWisdomCreateDto chineseWisdomDto)
        {
            await _service.CreateAsync(chineseWisdomDto);
            return StatusCode(StatusCodes.Status200OK);
        }
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync([FromForm] ChineseWisdomUpdateDto updateDto, int id)
        {
            if (id <= 0) return StatusCode(StatusCodes.Status400BadRequest);
