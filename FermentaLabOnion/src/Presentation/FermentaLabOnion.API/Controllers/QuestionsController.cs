...
namespace FermentaLabOnion.API.Controllers
{
    ...
    public class QuestionsController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromForm] QuestionCreateDto questionDto)
        {
            await _service.CreateAsync(questionDto);
            return StatusCode(StatusCodes.Status200OK);
        }
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync([FromForm] QuestionUpdateDto updateDto, int id)
        {
            if (id <= 0) return StatusCode(StatusCodes.Status400BadRequest);
