...
namespace FermentaLabOnion.API.Controllers
{
    ...
    [ApiController]
    public class CategoryTranslatesController : ControllerBase
    {
        private readonly ICategoryTranslateService _service;

        public CategoryTranslatesController(ICategoryTranslateService service)
        {
            _service = service;
        }
        [HttpGet]
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync([FromForm] CategoryTranslateUpdateDto updateDto, int id)
        {
            if (id <= 0) return StatusCode(StatusCodes.Status400BadRequest);
