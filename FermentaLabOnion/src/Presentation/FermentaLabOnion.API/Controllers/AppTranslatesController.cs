...
namespace FermentaLabOnion.API.Controllers
{
    ...
    {
        private readonly IAppTranslateService _service;

        public AppTranslatesController(IAppTranslateService service)
        {
            _service = service;
        }
        [HttpGet]
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromForm] AppTranslateCreateDto appDto)
        {
            await _service.CreateAsync(appDto);
            return StatusCode(StatusCodes.Status200OK);
        }
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync([FromForm] AppTranslateUpdateDto updateDto, int id)
        {
            if (id <= 0) return StatusCode(StatusCodes.Status400BadRequest);
