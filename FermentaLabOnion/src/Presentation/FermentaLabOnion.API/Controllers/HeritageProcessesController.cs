...
namespace FermentaLabOnion.API.Controllers
{
    ...
    [ApiController]
    public class HeritageProcessesController : ControllerBase
    {
        private readonly IHeritageProcessService _service;

        public HeritageProcessesController(IHeritageProcessService service)
        {
            _service = service;
        }
        [HttpGet]
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromForm] HeritageProcessCreateDto heritageProcessDto)
        {
            await _service.CreateAsync(heritageProcessDto);
            return StatusCode(StatusCodes.Status200OK);
        }
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync([FromForm] HeritageProcessUpdateDto updateDto, int id)
        {
            if (id <= 0) return StatusCode(StatusCodes.Status400BadRequest);
